 ...
 *  effect actually implemented - is ported. */
...
export const GLASS_FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    if (insideCircle > 0.0) {
      ...
      revealed = vec4(
        texture2D(uTexture2, distortedUv + direction * chromatic * 1.2).r,
        texture2D(uTexture2, distortedUv + direction * chromatic * 0.2).g,
        texture2D(uTexture2, distortedUv - direction * chromatic * 0.8).b,
        1.0
      );
      ...
      revealed = texture2D(uTexture2, uv2);
    }

    if (uProgress > 0.95) {
      revealed = mix(revealed, texture2D(uTexture2, uv2), (uProgress - 0.95) / 0.05);
    }

    vec4 previous = texture2D(uTexture1, uv1);
