...
import { sheetIconBadgeVariants, sheetIconVariants, sheetVariants } from "./folder.variants";
  ...
  return (
    <Magnetic actionArea="global" >
      <div id={id} className={sheetVariants({ color: sheet.color, side })}>
        <span id={`${id}-badge`} className={sheetIconBadgeVariants({ color: sheet.color })}>
          <sheet.Icon aria-hidden className={sheetIconVariants({ color: sheet.color })} />
        </span>
      </div>
    </Magnetic>
  );
